...
#> 
   ...
   Write-Host "Creating the AAD application (java-servlet-resource-api)"
   # create the application 
   $serviceAadApplication = New-AzureADApplication -DisplayName "java-servlet-resource-api" `
                                                   -HomePage "http://localhost:8080/msal4j-servlet-api" `
                                                   -PublicClient $False
   ...
   { 
    ...
    if ($scopes.Count -ge 0) 
    {
             $scope = CreateScope -value access_as_user  `
                -userConsentDisplayName "Access java-servlet-resource-api"  `
                -userConsentDescription "Allow the application to access java-servlet-resource-api on your behalf."  `
                -adminConsentDisplayName "Access java-servlet-resource-api"  `
   ...
   Write-Host "Done creating the service application (java-servlet-resource-api)"
   ...
   Add-Content -Value "<tr><td>service</td><td>$currentAppId</td><td><a href='$servicePortalUrl'>java-servlet-resource-api</a></td></tr>" -Path createdApps.html


   # Create the client AAD application
   Write-Host "Creating the AAD application (java-servlet-webapp-client)"
   ...
   $clientAppKey = $pw
   # create the application 
   $clientAadApplication = New-AzureADApplication -DisplayName "java-servlet-webapp-client" `
                                                  -HomePage "http://localhost:8080/" `
                                                  -ReplyUrls "http://localhost:8080/msal4j-servlet-client/auth/redirect" `
   ...
   Write-Host "Done creating the client application (java-servlet-webapp-client)"

   # URL of the AAD application in the Azure portal
   # Future? $clientPortalUrl = "https://portal.azure.com/#@"+$tenantName+"/blade/Microsoft_AAD_RegisteredApps/ApplicationMenuBlade/Overview/appId/"+$clientAadApplication.AppId+"/objectId/"+$clientAadApplication.ObjectId+"/isMSAApp/"
   $clientPortalUrl = "https://portal.azure.com/#blade/Microsoft_AAD_RegisteredApps/ApplicationMenuBlade/CallAnAPI/appId/"+$clientAadApplication.AppId+"/objectId/"+$clientAadApplication.ObjectId+"/isMSAApp/"
   Add-Content -Value "<tr><td>client</td><td>$currentAppId</td><td><a href='$clientPortalUrl'>java-servlet-webapp-client</a></td></tr>" -Path createdApps.html

   $requiredResourcesAccess = New-Object System.Collections.Generic.List[Microsoft.Open.AzureAD.Model.RequiredResourceAccess]

   # Add Required Resources Access (from 'client' to 'service')
   Write-Host "Getting access from 'client' to 'service'"
   $requiredPermissions = GetRequiredPermissions -applicationDisplayName "java-servlet-resource-api" `
   ...
   $configFile = $pwd.Path + "\..\resource-api\src\main\resources\authentication.properties"
   Write-Host "Updating the sample code ($configFile)"
   $dictionary = @{ "Enter_Your_Tenant_ID_Here" = $tenantId;"Enter_Your_Client_ID_Here" = $serviceAadApplication.AppId };
   ReplaceInTextFile -configFilePath $configFile -dictionary $dictionary

   # Update config file for 'client'
   $configFile = $pwd.Path + "\..\client-web-app\src\main\resources\authentication.properties"
   Write-Host "Updating the sample code ($configFile)"
   ...
   Write-Host "  - Go to your java-servlet-resource-api AAD app registration. Select the `Manifest` blade on the left. Set the 'accessTokenAcceptedVersion' to '2'. Save your changes." -ForegroundColor Red 
